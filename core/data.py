...
# TypedDict for safer data passing
class BrainData(TypedDict):
    vertices: np.ndarray      # (N, 3) float32
    faces: np.ndarray         # (M, 3) uint32
    normals: np.ndarray       # (N, 3) float32
    colors: np.ndarray        # (N, 3) float32 (Initial colors)
    curvature: np.ndarray     # (N,) float32
    color_frames: np.ndarray  # (N, T, 3) float32
    atlas_colors: np.ndarray  # (N, 3) float32
    traces: List[np.ndarray]  # List of 1D arrays
...
def _load_hemisphere(mesh_path, labels, label_colors):
    ...
    # Pick random active clusters
    active_clusters = np.random.choice(unique_labels, size=5, replace=False) if len(unique_labels) >= 5 else unique_labels
    ...
    # Trimesh processing for normals/curvature
...
def ensure_destrieux_downloaded():
    """Ensure Destrieux atlas files are present, bypassing SSL checks if needed."""
    ...
    for filename, url in files.items():
        ...
        if not os.path.exists(filepath):
            print(f"Downloading {filename} manually...")
            try:
                subprocess.run(["curl", "-k", "-L", "-o", filepath, url], check=True)
            except subprocess.CalledProcessError:
                print(f"Failed to download {filename}")

def load_brain_data() -> BrainData:
    """
    Fetch surface geometry and atlas data.
    """
    ...
    color_frames = np.concatenate([frames_l, frames_r], axis=0)
    atlas_colors = np.vstack([atlas_l, atlas_r])
